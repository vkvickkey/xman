  ...
  return (
    ...
    >
      ...
      <div className="w-full fixed z-[99] backdrop-blur-xl bg-black/40 border-b border-white/5 flex items-center gap-3 sm:h-[7vh] h-[10vh] px-6">
        ...
        ></i>
        <h1 className="text-xl text-white font-black tracking-tighter uppercase whitespace-nowrap overflow-hidden text-ellipsis max-w-[40%] sm:max-w-[30%]">
          {details[0]?.album?.name || "Album"}
        </h1>
      ...
      <div className="flex w-full bg-black pt-[15vh] sm:pt-[10vh] pb-[25vh] sm:pb-[35vh] text-white p-10 sm:p-3 sm:gap-3 min-h-screen overflow-y-auto sm:block flex-wrap gap-5 justify-center">
        {details?.map((d, i) => (
          <div
            title="Click to play"
            key={i}
            className="items-center justify-center relative hover:scale-[1.02] duration-300 w-[45%] flex mb-3 sm:mb-4 sm:w-full sm:flex sm:items-center sm:gap-3 rounded-2xl h-[120px] sm:h-[100px] cursor-pointer bg-white/5 border border-white/10 backdrop-blur-md hover:bg-white/10 hover:border-white/20 transition-all shadow-lg hover:shadow-purple-glow/20 group"
          >
            <div
              onClick={() => audioseter(i)}
              className="flex w-[85%] items-center h-full px-4"
            >
              <div className="relative flex-shrink-0">
                  <motion.img
                    initial={{ opacity: 0, scale: 0.8 }}
                    animate={{ opacity: 1, scale: 1 }}
                    className="w-[80px] h-[80px] sm:w-[70px] sm:h-[70px] rounded-xl object-cover shadow-xl border border-white/10"
                    src={d.image[2].url}
                    alt=""
                  />
                  {d.id === currentSong?.id && (
            ...
            <div className="flex flex-col items-center justify-center gap-4 pr-6">
              <i
                ...
                className={`text-2xl transition-all duration-300 hover:scale-125 cursor-pointer ${existingData?.find((element) => element?.id == d?.id) ? "text-p-magenta drop-shadow-[0_0_10px_rgba(191,64,255,0.6)] ri-heart-3-fill" : "text-white/20 ri-heart-3-line"}`}
              ></i>
              <i
                onClick={() =>
                  handleGenerateAudio({
